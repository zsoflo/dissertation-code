  ...
  //if the received code is valid, use switch to select action based on command_code
    ...
    switch (command_code) {
      ...
      case "Director": //Direct: director trial
        ...
        director_trial(command.target,command.foils,command.partner_id);
        ...
        //                response: director's label})
        // var target = command.target; // get target from the command dictionary/target_object
        // var foils = command.foils; // get the foils from the command dictionary/foils
        // var partner_id = command.partner_id; // get the partner's ID from the command dictionary
        // var target_as_html_string = '<img src="' + target + '.png" class="selected">'; // format the target as a displayed image
        // var foils_as_html_string = foils.map(function(foil) {return '<img src="' + foil + '.png">';}); // format the foils list as a displayed image array
        // var full_context = [target_as_html_string] + [foils_as_html_string]; // display the image target and the image foils
        break;
      case "Matcher": //Matcher: matcher trial
        ...
        matcher_trial(command.director_label,command.partner_id, command.target, command.foils);
        // get the director's label from the command dictionary
        // get the director's array from the command dictionary
        // get the partner's ID from the command dictionary
        break;
      case "Feedback": //Feedback: feedback for director or matcher
        ...
        display_feedback(command.score,command.target,command.label);
