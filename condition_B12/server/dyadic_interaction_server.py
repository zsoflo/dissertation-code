...
# In this experiment, the director is prompted with a target image and foil images (images array) 
# and asked to provide a label for the matcher, who then guesses what the target image was from the target image + foils array. 
# The array depends on the conditions. That means we need a list of possible conditions. 

condition = ['shape_different', 'color_different']

# And that means we need a list of target images. This is every image (16) because every image is a target once for each condition.
# 32 total trials.

targets = [
    ['yellow_rect', 'shape_different'], ['blue_rect', 'shape_different'], ['red_rect', 'shape_different'], ['grey_rect', 'shape_different'],
    ['yellow_blob', 'shape_different'], ['blue_blob', 'shape_different'], ['red_blob', 'shape_different'], ['grey_blob', 'shape_different'],
    ['yellow_oval', 'shape_different'], ['blue_oval', 'shape_different'], ['red_oval', 'shape_different'], ['grey_oval', 'shape_different'],
    ['yellow_star', 'shape_different'], ['blue_star', 'shape_different'], ['red_star', 'shape_different'], ['grey_star', 'shape_different'],
    ['yellow_rect', 'color_different'], ['blue_rect', 'color_different'], ['red_rect', 'color_different'], ['grey_rect', 'color_different'],
    ['yellow_blob', 'color_different'], ['blue_blob', 'color_different'], ['red_blob', 'color_different'], ['grey_blob', 'color_different'],
    ['yellow_oval', 'color_different'], ['blue_oval', 'color_different'], ['red_oval', 'color_different'], ['grey_oval', 'color_different'],
    ['yellow_star', 'color_different'], ['blue_star', 'color_different'], ['red_star', 'color_different'], ['grey_star', 'color_different'],
]
    

# Each image is a target twice: once in the shape-different condition,
# and once in the color-different condition.

# all_objects = [
#     'yellow_rect', 'blue_rect', 'red_rect', 'grey_rect',
#     'yellow_blob', 'blue_blob', 'red_blob', 'grey_blob',
#     'yellow_oval', 'blue_oval', 'red_oval', 'grey_oval',
#     'yellow_star', 'blue_star', 'red_star', 'grey_star',
# ]

# ask KS about difference between targets and all_objects

# Select the target from targets list.
# Sample without replacement.

# Target is selected, now select the respective foils.

def pick_foils(target, condition):
    colors = {'red', 'yellow', 'blue', 'grey'}
    shapes = {'blob', 'oval', 'star', 'rect'}
    if condition=='shape_different':
        color, shape = target.split('_') # 'red_blob' (color, shape) will be split at the '_' > 'red' 'blob'
        remaining_shapes = shapes - { shape } # the remaining shapes other than blob will be in 'remaining_shapes'
        return [color+'_'+s for s in remaining_shapes] # return the same color, 'red' + '_' + every element in 'remaining shapes'
        # this returns: 'red_rect', 'red_oval', 'red_star'
    elif condition=='color_different':
        color, shape = target.split('_')
        remaining_colors = colors - { color }
        return [c+'_'+shape for c in remaining_colors]
...
def enter_phase(client_id,phase):
    ...
    # Attempts to pair this client with anyone already in the waiting room
        ...
        if (len(unpaired_clients)%2 ==0): #If there are exactly 2 people now in unpaired_clients, pair them
            ...
            # Both participants work through a shared targets list
            # with both clients, then move them to the next phase
            shuffled_targets = shuffle(targets)
...
def start_interaction_trial(list_of_participants):
    ...
    if not(all_connected(list_of_participants)):
        ...
        if trial_counter>=ntrials:
            ...
            target_trial_type = target[1]
            foils = pick_foils(target_object, target_trial_type)
            for c in list_of_participants:
...
# matcher's array of options (the shared foils) and the director's participant ID
#so the matcher can record this in their data file for us.

# When director responds, relay the label, the target image, and the foils.
def handle_director_response(director_id,director_response):
    ...
    if not(all_connected([matcher_id])): #the usual check that everyone is still connected
        ...
        # shuffled_context = director_response['shuffled_context']
        # shuffled_elements = shuffled_context[1: -1].split('> <')
        # shuffled_elements = ['<' + i + '>' for i in shuffled_elements]
        target = director_response['target']
        foils = director_response['foils']
        send_message_by_id(matcher_id,
                            {
                                "command_type": "Matcher",
                                # "shuffled_context": shuffled_elements, 
...
def handle_matcher_response(matcher_id,matcher_response):
    ...
    if not(all_connected([director_id,matcher_id])):
        ...
        target_position = matcher_response['target_pos']
        print(label)
        print(target)
        print(target_position)
        print(guess)
        if target_position==guess:
            score=1
        else:
            score=0
        feedback = {
...
def send_instructions(client_id,phase):
    if phase=='Interaction':
        ...
        send_message_by_id(client_id,{
            "command_type":"Instructions",
            "instruction_type":"Interaction"
        })
...
PORT=9026 #this will run on port 9026
